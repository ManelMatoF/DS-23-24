...
public class ReleaseTest2 {
    ...
    static void setUp() {
        ...
        // https://musicbrainz.org/release/0173d    355-016f-4d28-8c57-cdb0b6fd30a0
    ...
    public void testToString() {
        Track track = new Track("1", "123", "Artist", "Title", 180);
        String expectedToString = "Track{trackPos='1', trackId='123', artist='Artist', title='Title', duration=180}";
        assertEquals(expectedToString, track.toString());
    }
    @Test
    void testGetReleaseId() {
        assertEquals("fcd7fed3-4523-49bc-a310-129a1f3b6bcc", madonnaAngelUKSingle.getReleaseId());
        assertEquals("0173d355-016f-4d28-8c57-cdb0b6fd30a0", beatlesPennyLanePromo.getReleaseId());
    }

    @Test
    void testGetArtist() {
        assertEquals("Beatles", beatlesSFFPennyLane.getArtist());
        assertEquals("Madonna", madonnaAngelUKSingle.getArtist());
    }

    @Test
    void testGetTitle() {
        assertEquals("Strawberry Fields Forever / Penny Lane", beatlesSFFPennyLane.getTitle());
        assertEquals("Penny Lane / Strawberry Fields Forever (Promo)", beatlesPennyLanePromo.getTitle());
    }

    @Test
    void testGetTracklist() {
        assertNotNull(beatlesSFFPennyLane.getTracklist());
        assertNotNull(madonnaAngelUKSingle.getTracklist());

        assertFalse(beatlesSFFPennyLane.getTracklist().isEmpty());
        assertFalse(madonnaAngelUKSingle.getTracklist().isEmpty());
    }
